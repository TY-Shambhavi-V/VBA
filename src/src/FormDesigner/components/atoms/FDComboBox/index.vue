...
export default class FDComboBox extends Mixins(FdControlVue) {
  ...
  test1 (controlProp: controlProperties, choice: string) {
    const height = choice === 'right' ? `left: ${-controlProp.Width! - 5}px`
      : choice === 'left' ? `left: ${controlProp.Width! + 5}px`
        : choice === 'top' ? `top: ${controlProp.Height! + 1}px`
          : choice === 'bottom' ? `bottom: ${controlProp.Height! - 1}px` : ''
    console.log(height)
    switch (choice) {
      case 'right':
        console.log('object')
  ...
  protected get itemsStyleObj (): Partial<CSSStyleDeclaration> {
    const controlProp = this.properties
    if (this.top < (window.innerHeight * (3 / 4) - controlProp.Height!)) {
      this.test1(controlProp, 'right')
    }
    return {
      width:
