        ...
        <div class="wrapper1">
          ...
          <div class="frame">
            <div v-for="(value, index) in tabOrderList" :key="value.Name"
                ...
                @keydown="selectedItem(index, $event), sample()" ref="frameRef"
>
              <button
                class="inside-frame"
                :class="{ 'active-item': currentIndex.includes(index) }"
                @mousedown="selectedTab(index), sample()"
...
export default class TabStripTabOrderModal extends FdDialogDragVue {
  ...
  @Ref('frameRef') frameRef: HTMLDivElement[];
  ...
  focusSelectedControl () {
    debugger
    console.log('focused')
    if (this.frameRef[0].children[0] && this.frameRef[0].children[0] instanceof HTMLButtonElement) {
      this.frameRef[0].children[0].focus()
    }
  }
  created () {
    EventBus.$on(
